...
class TestAdmin(unittest.TestCase):
    def test_wildcard_admin_is_critical(self):
        f = rules.run(acct_from_user_policy([{"Effect": "Allow", "Action": "*", "Resource": "*"}]))
...
class TestPrivescPaths(unittest.TestCase):
    def test_create_policy_version(self):
        f = rules.run(acct_from_user_policy([{"Effect": "Allow", "Action": "iam:CreatePolicyVersion", "Resource": "*"}]))
    ...
    def test_passrole_lambda_needs_all_three_actions(self):
        two = rules.run(acct_from_user_policy([{"Effect": "Allow", "Action": ["iam:PassRole", "lambda:CreateFunction"], "Resource": "*"}]))
        self.assertFalse(any("Lambda function and run it" in t for t in titles(two)))
        three = rules.run(acct_from_user_policy([{"Effect": "Allow", "Action": ["iam:PassRole", "lambda:CreateFunction", "lambda:InvokeFunction"], "Resource": "*"}]))
    ...
    def test_passrole_ec2_via_group(self):
        ...
        self.assertTrue(any("new EC2 instance" in t for t in titles(rules.run(acct))))

    def test_iam_star_reported_once_not_as_every_path(self):
        f = rules.run(acct_from_user_policy([{"Effect": "Allow", "Action": "iam:*", "Resource": "*"}]))
    ...
    def test_resource_scoped_grant_is_medium(self):
        f = rules.run(acct_from_user_policy([{"Effect": "Allow", "Action": "iam:UpdateLoginProfile", "Resource": "arn:aws:iam::1:user/other"}]))
    ...
    def test_least_privilege_user_has_no_findings(self):
        f = rules.run(acct_from_user_policy([{"Effect": "Allow", "Action": ["s3:GetObject"], "Resource": "arn:aws:s3:::b/*"}]))
        self.assertEqual(f, [])
...
class TestTrust(unittest.TestCase):
    ...
    def test_public_role_is_critical(self):
        f = rules.run(self._role([{"Effect": "Allow", "Principal": {"AWS": "*"}, "Action": "sts:AssumeRole"}]))
    ...
    def test_cross_account_trust_is_flagged(self):
        f = rules.run(self._role([{"Effect": "Allow", "Principal": {"AWS": "arn:aws:iam::999988887777:root"}, "Action": "sts:AssumeRole"}]))
        self.assertTrue(any("external account" in t for t in titles(f)))

    def test_same_account_trust_is_not_flagged(self):
        f = rules.run(self._role([{"Effect": "Allow", "Principal": {"AWS": "arn:aws:iam::111122223333:role/other"}, "Action": "sts:AssumeRole"}]))
        self.assertFalse(any("external account" in t for t in titles(f)))

    def test_service_trust_is_not_flagged(self):
        f = rules.run(self._role([{"Effect": "Allow", "Principal": {"Service": "lambda.amazonaws.com"}, "Action": "sts:AssumeRole"}]))
        self.assertEqual(f, [])

    def test_federated_without_condition_is_flagged(self):
        f = rules.run(self._role([{"Effect": "Allow", "Principal": {"Federated": "arn:aws:iam::111122223333:saml-provider/x"}, "Action": "sts:AssumeRoleWithSAML"}]))
...
class TestWildcards(unittest.TestCase):
    ...
    def test_s3_service_wildcard_is_medium(self):
        f = rules.run(acct_from_user_policy([{"Effect": "Allow", "Action": "s3:*", "Resource": "*"}]))
        hit = [x for x in f if "Full control of s3" in x["title"]]
        self.assertTrue(hit)
        self.assertEqual(hit[0]["severity"], "medium")

    def test_non_sensitive_wildcard_is_ignored(self):
        f = rules.run(acct_from_user_policy([{"Effect": "Allow", "Action": "cloudwatch:*", "Resource": "*"}]))
        self.assertEqual(f, [])
